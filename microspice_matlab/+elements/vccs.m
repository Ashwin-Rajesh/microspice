classdef vccs < elements.element
    properties
        g                   (1, 1)      double;
    end
    ...
    methods
        function ret = read_spice(obj, spice_inp)
            ...
            obj.id = inp(1);
            obj.nodes = [inp(2), inp(3), inp(4), inp(5)];
            obj.g = parser.parse_number(inp(6));
            
            if(isnan(obj.g))
                ret = error.inp_error.inp_error(file_name, line_num, "Argument '" + inp(6) + "' is not valid numeric format");
            end
            return
        end
